# -*- encoding: utf-8 -*-

=begin
#Redocly Museum API

#Imaginary, but delightful Museum API for interacting with museum services and information. Built with love by Redocly.

The version of the OpenAPI document: 1.2.1
Contact: team@redocly.com
Generated by: https://openapi-generator.tech
Generator version: 7.11.0

=end

$:.push File.expand_path("../lib", __FILE__)
require "openapimuseum_client/version"

Gem::Specification.new do |s|
  s.name        = "openapimuseum_client"
  s.version     = OpenapiMuseumClient::VERSION
  s.platform    = Gem::Platform::RUBY
  s.authors     = ["OpenAPI-Generator"]
  s.email       = ["team@redocly.com"]
  s.homepage    = "https://openapi-generator.tech"
  s.summary     = "Redocly Museum API Ruby Gem"
  s.description = "Imaginary, but delightful Museum API for interacting with museum services and information. Built with love by Redocly."
  s.license     = "Unlicense"
  s.required_ruby_version = ">= 2.7"
  s.metadata    = {}

  s.add_runtime_dependency 'typhoeus', '~> 1.0', '>= 1.0.1'

  s.add_development_dependency 'rspec', '~> 3.6', '>= 3.6.0'

  s.files         = `find *`.split("\n").uniq.sort.select { |f| !f.empty? }
  s.test_files    = `find spec/*`.split("\n")
  s.executables   = []
  s.require_paths = ["lib"]
end
